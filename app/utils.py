...
def get_birth_death_date(text):
    ...
    # birth date and age
    match = re.search(r'{{\s*birth[\s_]*date and age\s*\|[^\d\}]*(\d+\|\d+\|\d+)', text, flags=re.I)
    if match:
        # living
        return parse_date(match.group(1).replace('|', '-')), None

    # birth date
    match = re.search(r'{{\s*birth[\s_]*date\s*\|[^\d\}]*(\d+\|\d+\|\d+)', text, flags=re.I)
    if match:
    ...
    if not birth_date:
        match = re.search(r'{{\s*birth[\s_]*year and age\s*\|[^\d\}]*(\d+)', text, flags=re.I)
        if match:
    ...
    if not death_date:
        match = re.search(r'{{\s*death[\s_]*date\s*\|[^\d\}]*(\d+)\|(\d+)\|(\d+)', text, flags=re.I)
        if match:
            death_date = datetime.date(int(match.group(1)), int(match.group(2)), int(match.group(3)))
    if not death_date:
        match = re.search(r'{{\s*death[\s_]*date\s*\|[^\d\}]*(\d+)\|(\d+)', text, flags=re.I)
        if match:
            death_date = datetime.date(int(match.group(1)), int(match.group(2)), 1)
    if not death_date:
        match = re.search(r'{{\s*death[\s_]*date\s*\|[^\d\}]*(\d+)', text, flags=re.I)
        if match:
    ...
    if not death_date:
        match = re.search(r'{{\s*death[\s_]*year and age\s*\|[^\d\}]*(\d+)\|\d+\|(\d+)', text, flags=re.I)
        if match:
            death_date = datetime.date(int(match.group(1)), int(match.group(2)), 1)
        else:
            match = re.search(r'{{\s*death[\s_]*year and age\s*\|[^\d\}]*(\d+)', text, flags=re.I)
            if match:
                death_date = datetime.date(int(match.group(1)), 1, 1)
